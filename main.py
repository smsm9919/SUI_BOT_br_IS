...
import os, time, math, random, signal, sys, traceback, logging, uuid, threading, csv, json
...
def _reset_after_close(reason, prev_side=None):
    global LAST_CLOSE_BAR_TS, POST_CHOP_BLOCK_ACTIVE, POST_CHOP_BLOCK_UNTIL_BAR, _active_side
...
def can_execute_now():
    return (time.time() - _last_entry_ts) >= COOLDOWN_SEC
...
def maybe_execute(df, council, signals, golden, ind, balance):
    global _last_entry_ts, _active_side, _entry_attempt_count, _last_execution_log
    ...
    try:
        ...
        STATE.update({
            ...
            "qty": qty,
            "trading_mode": mode,
            "entry_strength": council.get('score', 0),
            "bars": 0,
            "age_min": 0
        })
        
        _last_execution_log = f"ENTRY {side} {mode} @ {price:.6f}"
        return True
    except Exception as e:
...
def manage_position(state, df, ind, council, mode):
    ...
    if state.get("bars", 0) > 0:
        state["age_min"] = state["bars"] * 15  # تقدير بالدقائق
    
    # SCALP Management
    if mode == 'SCALP':
        if upl_pct >= 0.35:
            close_all("SCALP_TP", state)
        elif state.get("age_min", 0) >= 20:  # 20 دقيقة
            close_all("SCALP_TIMEOUT", state)
        elif ind.get('macd_hist', 0) < -0.001:  # مبسط بدل EVX
    ...
    log_manage(
        side=side, 
        upnl_pct=upl_pct/100,
        trail_active=tm.get("trailing_active", False),
        be_active=tm.get("break_even_moved", False),
        partial_done=tm.get("partial_taken", False),
        hold_tp=len(tm.get("targets_hit", [])) > 0,
        notes=f"Age: {state.get('age_min', 0)}min"
    )

def close_all(reason, state):
    global _active_side, compound_pnl
...
def elite_trade_loop_enhanced():
    global _last_entry_ts, _active_side, compound_pnl, LAST_DECISION_BAR_TS
...
# =================== دوال التنفيذ الأساسية القديمة (للتوافق) ===================
...
def metrics():
    return jsonify({
        ...
        "execution_stats": {
            ...
            "entry_attempts": _entry_attempt_count,
            "last_execution": _last_execution_log
        }
    })
